...
def render_en1997() -> None:
    ...
    # --- DESIGN APPROACH & PARTIAL FACTOR SELECTION BAR ---
    ...
    # Map partial factors based on Design Approach selection
    da_code = design_approach.split(" - ")[0]
    ...
    with col_design_results:
        ...
        # Eccentricity & Effective Area
        e_b = m_ed / v_ed if v_ed > 0 else 0.0
        b_eff = width_b - (2 * e_b)
        l_eff = length_l  # Assuming uniaxial eccentricity
        ...
        if phi_d_deg > 0:
            ...
            iq = max(0.0, (1.0 - H_ed_val := h_ed / (v_ed + a_eff * c_d / math.tan(phi_d_rad))) ** m_shape)
        ...
        # Ultimate Bearing Resistance q_rd and R_d
        q_rd = (c_d * nc * sc * ic) + (q_overburden * nq * sq * iq) + (0.5 * gamma_d * b_eff * ngamma * sgamma * igamma)
        r_d = (q_rd * a_eff) / gamma_r_b  # kN

        # Sliding Resistance Check (EN 1997-1 cl 6.5.3)
        ...
        summary_df = pd.DataFrame(
            [
                {
                    "Design Check": "Bearing Capacity ULS (V_Ed <= R_d)",
                    "Applied Load": f"V_Ed = {v_ed:.1f} kN",
                    "Resistance": f"R_d = {r_d:.1f} kN",
                    "Utilization": f"{ur_bearing:.2f}",
                    "Status": "PASS" if ur_bearing <= 1.0 else "FAIL",
                },
                {
                    "Design Check": "Sliding Resistance ULS (H_Ed <= R_h,d)",
                    "Applied Load": f"H_Ed = {h_ed:.1f} kN",
                    "Resistance": f"R_h,d = {r_h_d:.1f} kN",
                    "Utilization": f"{ur_sliding:.2f}",
                    "Status": "PASS" if ur_sliding <= 1.0 else "FAIL",
                },
                {
                    "Design Check": "Eccentricity Limit (e <= B/6)",
                    "Calculated": f"e = {e_b:.3f} m",
                    "Limit": f"B/6 = {width_b / 6:.3f} m",
                    "Utilization": f"{e_b / (width_b / 6):.2f}",
                    "Status": "NO UPLIFT" if e_b <= width_b / 6 else "PARTIAL UPLIFT",
                },
            ]
        )
        st.dataframe(summary_df, use_container_width=True, hide_index=True)

        # Bearing Capacity Curve vs Footing Width B
        ...
        chart_df = pd.DataFrame(
            {"Footing Width B (m)": widths, "Bearing Resistance R_d [kN]": rd_list}
        ).set_index("Footing Width B (m)")
