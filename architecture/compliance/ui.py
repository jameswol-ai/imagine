...
def render_compliance() -> None:
    ...
    with left:
        ...
        floors = st.number_input("Storeys", min_value=1, value=6, step=1, key="comp_storeys")
        occupants = st.number_input("Peak occupants", min_value=1, value=350, step=10, key="comp_occupants")
    ...
    with right:
        if audit:
            ...
            if passed == len(checks):
                st.success(f"{code} screening complete: {passed}/{len(checks)} checks pass.")
            else:
                st.warning(f"{code} screening complete: {passed}/{len(checks)} checks pass. Review the flagged items.")
        else:
            st.info("Run the screening to record the current compliance assessment.")
        st.dataframe(pd.DataFrame(checks), use_container_width=True, hide_index=True)

        st.subheader("Life-safety planning indicators")
