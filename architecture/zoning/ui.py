...
def render_zoning() -> None:
    ...
    with left:
        ...
        lot_area = st.number_input("Lot area (m²)", min_value=100.0, value=float(_brief_value("site_area_m2", 5000.0)), step=100.0, key="zoning_lot_area")
        proposed_gfa = st.number_input("Proposed GFA (m²)", min_value=0.0, value=float(st.session_state.get("architecture_assessment").program_gross_area_m2 if st.session_state.get("architecture_assessment") else 12000.0), step=250.0, key="zoning_proposed_gfa")
        max_far = st.number_input("Maximum FAR", min_value=0.1, value=float(_brief_value("max_far", 4.5)), step=0.1, key="zoning_max_far")
        footprint_default = float(st.session_state.get("site_plan_result", {}).get("buildable_area_m2", 1800.0))
        footprint = st.number_input("Proposed footprint (m²)", min_value=0.0, value=footprint_default, step=50.0, key="zoning_footprint")
        ...
        max_height = st.number_input("Maximum height (m)", min_value=1.0, value=float(_brief_value("max_height_m", 45.0)), step=1.0, key="zoning_max_height")
    ...
    with right:
        if audit:
            ...
            message = f"Screening complete for {district}: {passed}/{len(checks)} checks pass."
            st.success(message) if passed == len(checks) else st.warning(message + " Review failed items before progressing.")
        else:
            st.info("Adjust the proposal inputs and run the screening to evaluate the current design envelope.")
        st.dataframe(pd.DataFrame([{"Check": n, "Proposed / Required": v, "Status": "PASS" if ok else "REVIEW"} for n, ok, v in checks]), use_container_width=True, hide_index=True)
        ...
        st.subheader("Architecture workflow handoff")
        assessment = st.session_state.get("architecture_assessment")
        if assessment:
            st.dataframe(pd.DataFrame([
                {"Input": "Assistant program GFA", "Value": f"{assessment.program_gross_area_m2:,.0f} m²"},
                {"Input": "Assistant FAR", "Value": f"{assessment.far_used:.2f}"},
                {"Input": "Assistant footprint", "Value": f"{assessment.buildable_footprint_m2:,.0f} m²"},
            ]), use_container_width=True, hide_index=True)
        else:
            st.info("Run Architecture Assistant first to populate the shared design brief.")

        st.warning("FAR, coverage, height and parking values are user-supplied screening constraints, not legal determinations.")
