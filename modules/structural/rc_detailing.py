...
def render() -> None:
    ...
    st.dataframe(pd.DataFrame([["Section", f"{width:.0f} x {depth:.0f} mm"], ["Cover", cover,], ["Bar arrangement", f"{count}T{diameter:.0f}"], ["Nominal spacing", spacing], ["Clear spacing", clear_spacing]], columns=["Item", "Value"]), use_container_width=True, hide_index=True)
    if clear_spacing <= 0:
