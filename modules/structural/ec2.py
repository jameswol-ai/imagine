...
class ConcreteDesignProperties:
    ...
    def __post_init__(self) -> None:
        if self.fck_mpa <= 0:
            raise ValueError("fck_mpa must be greater than zero")
        if self.gamma_c <= 0:
            raise ValueError("gamma_c must be greater than zero")
        if self.alpha_cc <= 0:
            raise ValueError("alpha_cc must be greater than zero")
    ...
    def fctm_mpa(self) -> float:
        if self.fck_mpa <= 50.0:
            return 0.30 * self.fck_mpa ** (2.0 / 3.0)
...
class SteelDesignProperties:
    ...
    def __post_init__(self) -> None:
        if self.fyk_mpa <= 0:
            raise ValueError("fyk_mpa must be greater than zero")
        if self.gamma_s <= 0:
            raise ValueError("gamma_s must be greater than zero")
...
def effective_depth_mm(
    ...
    """Return effective depth to the centroid of tension reinforcement."""
    values = {
    ...
    if d <= 0:
        raise ValueError("effective depth must be greater than zero")
    return d


def minimum_tension_reinforcement_mm2(
    width_mm: float,
    effective_depth_mm_value: float,
    fck_mpa: float,
    fyk_mpa: float,
) -> float:
    """Return the common EC2 beam/slab minimum tension reinforcement expression."""
    if min(width_mm, effective_depth_mm_value, fck_mpa, fyk_mpa) <= 0:
        raise ValueError("width, effective depth, fck and fyk must be greater than zero")
    fctm = ConcreteDesignProperties(fck_mpa).fctm_mpa
    return max(
        0.26 * fctm / fyk_mpa * width_mm * effective_depth_mm_value,
        0.0013 * width_mm * effective_depth_mm_value,
    )


def maximum_longitudinal_reinforcement_mm2(width_mm: float, depth_mm: float) -> float:
    """Return the common 4% gross-section reinforcement screening limit."""
    if width_mm <= 0 or depth_mm <= 0:
        raise ValueError("width and depth must be greater than zero")
    return 0.04 * width_mm * depth_mm


def required_flexural_reinforcement_mm2(
    moment_kn_m: float,
    width_mm: float,
    effective_depth_mm_value: float,
    fyd_mpa: float,
    *,
    ...
    """Estimate tension steel from MEd = As * fyd * z.

    This is a rectangular-section screening expression. Neutral-axis limits,
    compression reinforcement, ductility and redistribution requirements are
    outside this primitive.
    """
    if moment_kn_m < 0:
        raise ValueError("moment_kn_m cannot be negative")
    if min(width_mm, effective_depth_mm_value, fyd_mpa) <= 0:
        raise ValueError("width, effective depth and fyd must be greater than zero")
    if not 0 < z_factor <= 1:
        raise ValueError("z_factor must be between zero and one")
...
def provided_bar_area_mm2(bar_dia_mm: float, spacing_mm: float | None = None, width_mm: float = 1000.0) -> float:
    """Return one-bar area, or reinforcement area within a strip width."""
    if bar_dia_mm <= 0:
        raise ValueError("bar_dia_mm must be greater than zero")
    area = math.pi * bar_dia_mm**2 / 4.0
    if spacing_mm is None:
        return area
    if spacing_mm <= 0 or width_mm <= 0:
        raise ValueError("spacing and width must be greater than zero")
...
def vrdc_mpa(
    ...
    """Return the EC2-style V_Rd,c screening shear stress in MPa."""
    if min(fck_mpa, width_mm, effective_depth_mm_value, gamma_c) <= 0:
        raise ValueError("fck, width, effective depth and gamma_c must be greater than zero")
    if as_tension_mm2 < 0:
        raise ValueError("as_tension_mm2 cannot be negative")

    rho_l = min(as_tension_mm2 / (width_mm * effective_depth_mm_value), 0.02)
    k = min(2.0, 1.0 + math.sqrt(200.0 / effective_depth_mm_value))
    c_rd_c = 0.18 / gamma_c
    return c_rd_c * k * (100.0 * rho_l * fck_mpa) ** (1.0 / 3.0)
