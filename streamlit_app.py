...
def search_specs(query: str, domain: str = "All domains") -> list[ModuleSpec]:
    ...
    for spec in candidates:
        ...
        if phrase_hit:
            score += 120
        if normalized == spec.label.casefold() or normalized == spec.route.casefold():
            score += 180
...
def render_sidebar() -> None:
    with st.sidebar:
        st.markdown(
            '<div class="imagine-brand"><div class="imagine-brand-title">IMAGINE</div><div class="imagine-brand-subtitle">Integrated Architecture, Engineering & Construction Engine</div></div>',
            unsafe_allow_html=True,
        )
        st.divider()
        ...
        st.caption("Search results appear in the workspace. Module navigation is intentionally kept out of the sidebar.")


def render_header(title: str, subtitle: str, breadcrumb: str) -> None:
    st.markdown(
        f'<div class="imagine-header"><div class="imagine-header-title">{title}</div><div class="imagine-header-subtitle">{subtitle}</div><div class="imagine-breadcrumb">{breadcrumb}</div></div>',
        unsafe_allow_html=True,
    )
...
def render_search_results() -> bool:
    ...
    for index, spec in enumerate(matches[:30]):
        with cols[index % 3]:
            ...
            st.markdown(
                f'<div class="imagine-panel"><div class="imagine-panel-title">{spec.label}</div><div class="imagine-panel-description">{spec.section} · {status}<br>{spec.module_path or "Built-in workspace"}</div></div>',
                unsafe_allow_html=True,
            )
...
def render_overview() -> None:
    ...
    st.markdown(
        '<div class="imagine-hero"><div class="imagine-hero-title">Design. Engineer. Build. Operate.</div><div class="imagine-hero-copy">A unified AEC workspace for project delivery, engineering decision support, BIM coordination, cost control, construction management and operational intelligence. Search the workspace you need, then work directly inside its domain module.</div></div>',
        unsafe_allow_html=True,
    )
    ...
    with left:
        ...
        if projects:
            rows = [
                {
                    "Project": getattr(p, "name", "Unnamed project"),
    ...
    if recent:
        ...
        for index, route in enumerate(recent):
            spec = spec_for_route(route)
            if not spec:
                continue
            with cols[index % len(cols)]:
                st.markdown(
                    f'<div class="imagine-panel"><div class="imagine-panel-title">{spec.label}</div><div class="imagine-panel-description">{spec.section} · {"Ready" if spec.implemented else "Registered"}</div></div>',
                    unsafe_allow_html=True,
                )
                if st.button("Open", key=f"recent_open_{route}", use_container_width=True):
                    set_active_route(route)
                    st.rerun()
    else:
        st.markdown('<div class="imagine-search-hint">Use Workspace Search in the sidebar to open any registered engineering or delivery workspace.</div>', unsafe_allow_html=True)
